...
dHANDLES.LTSAfig = figure; colormap(dHANDLES.LTSAfig, jet)
set(dHANDLES.LTSAfig,'name','LTSA and time series',...
    'WindowKeyReleaseFcn',@keyAction)
dHANDLES.hbLTSA = brush(dHANDLES.LTSAfig);
set(dHANDLES.hbLTSA,'ActionPostCallback',{@brushAction,'brushH'})

dHANDLES.RMSvPPfig = figure;
set(dHANDLES.RMSvPPfig,'name',...
    sprintf('RL pp vs. RL rms (left shift by %d)',p.threshRL),...
    'WindowKeyReleaseFcn',@keyAction)
dHANDLES.hbRMSvPP = brush(dHANDLES.RMSvPPfig);
set(dHANDLES.hbRMSvPP,'ActionPostCallback',{@brushAction,'brushH'})

dHANDLES.PPvFreqfig = figure;
set(dHANDLES.PPvFreqfig,'name',...
    sprintf('RL rms vs. Peak freq. (left shift by %d)',p.threshRL),...
    'WindowKeyReleaseFcn',@keyAction)

dHANDLES.hbPPvFreq = brush(dHANDLES.PPvFreqfig);
set(dHANDLES.hbPPvFreq,'ActionPostCallback',{@brushAction,'brushH'})

dHANDLES.spectrafig = figure;
set(dHANDLES.spectrafig,'name','Frequency Spectra',...
        'WindowKeyReleaseFcn',@keyAction)

dHANDLES.wavefig = figure;
