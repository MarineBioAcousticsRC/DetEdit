...
if ~ isempty(MTT)
    ...
    if ~isempty(bdI)
        ...
        if exist('f','var')
            save(fullfile(outDir,outFileTPWS),'f','MTT','MPP','MSN','MSP','tfraw','-v7.3')
        else
            disp('no frequency vector available');
            save(fullfile(outDir,outFileTPWS),'MTT','MPP','MSN','MSP','tfraw','-v7.3')
        end
